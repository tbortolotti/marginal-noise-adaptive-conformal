...
if [[ $CONF == 700 ]]; then
  ...
  CONTAMINATION_LIST=("uniform")
  N_TRAIN_LIST=(10000)
  #N_CAL_LIST=(500 1000 2000 5000 10000 50000)
  N_CAL_LIST=(10000 50000)
                  ...
                  for N_TRAIN in "${N_TRAIN_LIST[@]}"; do
                    for N_CAL in "${N_CAL_LIST[@]}"; do
                      for GAMMA in "${GAMMA_LIST[@]}"; do
                          ...
                          # Script to be run
                          SCRIPT="exp_T_estimation.sh $CONF $DATA $NUM_VAR $K $SIGNAL $MODEL $EPSILON $NU $CONTAMINATION $N_TRAIN $N_CAL $GAMMA $SEED"
                          # Define job name
                          OUTF=$LOGS"/"$JOBN".out"
                          ERRF=$LOGS"/"$JOBN".err"
