...
def run_experiment(random_state):
    ...
    history_sll_alt = train_alternate(model_NN_sll_alt, X_torch, Y_obs_torch, I_torch, n_epochs=50, n_grad_steps=50, batch_size=128, lr=5e-2, verbose=False)
    ...
    history_alt = train_alternate(model_NN_alt, X_torch, Y_obs_torch, I_torch, n_epochs=50, n_grad_steps=50, batch_size=128, lr=5e-2, verbose=False)
    ...
    if contamination_exp_flag:
        ...
        model_NN = NoisyLabelNet(input_dim=num_var, K=K, hidden_dims=[16, 8], contamination_model_="general", epsilon_init=epsilon_init)
        train_alternate(model_NN, X_torch, Y_obs_torch, I_torch, n_epochs=50, n_grad_steps=50, batch_size=128, lr=5e-2, verbose=False)
        ...
        train_alternate(model_NN_sll, X_torch, Y_obs_torch, I_torch, n_epochs=50, n_grad_steps=50, batch_size=128, lr=5e-2, verbose=False)
