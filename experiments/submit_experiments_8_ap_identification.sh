...
if [[ $CONF == 800 ]]; then
  ...
  EPSILON_LIST=(0.1)
  CONTAMINATION_LIST=("uniform")
  N_TRAIN1_LIST=(100 200 500 1000 5000 10000 20000 50000)
  N_TRAIN2_LIST=(10000)
  SEED_LIST=$(seq 1 5)
                          ...
                          for CONTAMINATION in "${CONTAMINATION_LIST[@]}"; do
                            for N_TRAIN1 in "${N_TRAIN1_LIST[@]}"; do
                              for N_TRAIN2 in "${N_TRAIN2_LIST[@]}"; do
                                JOBN="exp"$CONF"/"$DATA"_p"$NUM_VAR"_K"$K"_PI"$PI"_DSHIFT"$DSHIFT"_CSCALE"$C_SCALE"_SEASY"$SEASY"_SHARD"$SHARD"_REASY"$REASY"_RHARD"$RHARD"_flipy"$FLIPY"_eps"$EPSILON"_"$CONTAMINATION"_nt1_"$N_TRAIN1"_nt2_"$N_TRAIN2"_seed"$SEED
                                ...
                                if [[ $COMPLETE -eq 0 ]]; then
                                  # Script to be run
                                  SCRIPT="exp_ap_identification.sh $CONF $DATA $NUM_VAR $K $PI $DSHIFT $C_SCALE $SEASY $SHARD $REASY $RHARD $FLIPY $EPSILON $CONTAMINATION $N_TRAIN1 $N_TRAIN2 $SEED"
                                  # Define job name
                                  OUTF=$LOGS"/"$JOBN".out"
                                  ERRF=$LOGS"/"$JOBN".err"
