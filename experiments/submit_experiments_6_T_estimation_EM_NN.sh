...
if [[ $CONF == 620 ]]; then
  ...
  N_CLEAN_LIST=(100 500 1000 5000)
              ...
              for EPSILON in "${EPSILON_LIST[@]}"; do
                for NU in "${NU_LIST[@]}"; do
                  for CONTAMINATION in "${CONTAMINATION_LIST[@]}"; do
                    JOBN="exp"$CONF"/"$DATA"_p"$NUM_VAR"_K"$K"_n"$N"_ncl"$N_CLEAN"_eps"$EPSILON"_nu"$NU"_"$CONTAMINATION"_seed"$SEED
                    ...
                    if [[ $COMPLETE -eq 0 ]]; then
                      # Script to be run
                      SCRIPT="exp_T_estimation_EM_NN.sh $CONF $DATA $NUM_VAR $K $N $N_CLEAN $RANDOM_FLAG $EPSILON $NU $CONTAMINATION $SEED"
                      # Define job name
                      OUTF=$LOGS"/"$JOBN".out"
                      ERRF=$LOGS"/"$JOBN".err"
