...
#' ---------------------------------------------------------------------------------------------------------------------
#### Experiment 711: Impact of the number of clean samples ------------------------
...
make_figure_712 <- function(exp.num, plot.alpha, plot.data="synthetic1", plot.K=4, plot.guarantee="marginal", save_plots=FALSE, reload=FALSE,
                            plot.contamination="uniform",
...
exp.num <- 712
plot.data <- "synthetic6"
make_figure_712(exp.num=exp.num, plot.alpha=plot.alpha, plot.data=plot.data, plot.K=plot.K,
                plot.guarantee="marginal",
                plot.contamination=plot.contamination,
                plot.n_train=plot.n_train, plot.pi_clean=plot.pi_clean,
                plot.epsilon=plot.epsilon, save_plots=FALSE, reload=TRUE)

#### Experiment 713: Impact of the number of training samples ------------------------
#' Plot marginal coverage as function of the number of calibration samples,
#' increasing the size of the training set

make_figure_713 <- function(exp.num, plot.alpha, plot.data="synthetic1", plot.K=4, plot.guarantee="marginal", save_plots=FALSE, reload=FALSE,
                            plot.contamination="uniform",
                            plot.n_train, plot.n_clean, plot.pi_clean,
...
make_figure_714 <- function(exp.num, plot.alpha, plot.data="synthetic1", plot.K=4, plot.guarantee="marginal", save_plots=FALSE, reload=FALSE,
                            plot.contamination="uniform",
                            plot.n_train, plot.pi_clean,
                            plot.epsilon) {
  if(reload) {
...
exp.num <- 714
plot.data <- "synthetic6"
make_figure_714(exp.num=exp.num, plot.alpha=plot.alpha, plot.data=plot.data, plot.K=plot.K,
                plot.guarantee="marginal",
                plot.contamination=plot.contamination,
                plot.n_train=plot.n_train, plot.pi_clean=plot.pi_clean,
                plot.epsilon=plot.epsilon, save_plots=FALSE, reload=TRUE)
...
init_settings <- function(contamination_exp_flag=FALSE) {
  cbPalette <<- c("grey50", "#E69F00", "#56B4E9", "#009E73", "#8A2BE2", "#0072B2", "#D55E00", "#CC79A7", "#20B2AA", "#F0E442")
  ...
  if(contamination_exp_flag){
    ...
    method.values <<- c("NN cifar light gen", "NN cifar SLL")
    ...
    method.values <<- c("NN cifar light", "NN cifar SLL")
    method.labels <<- c("NN", "NNs")
...
load_data <- function(exp.num, from_cluster=TRUE, contamination_exp_flag) {
  if(from_cluster) {
    idir <- sprintf("results_hpc/exp%d", exp.num)
  } else {
    idir <- sprintf("results/exp%d", exp.num)
  ...
  if(contamination_exp_flag){
    summary <- results %>%
      pivot_longer(c("frobenius_d"), names_to = "Key", values_to = "Value") %>%
...
make_figure_821 <- function(exp.num,plot.data="cifar10",
                            ...
                            plot.n_clean=500,
                            plot.pi_clean=0,
                            plot.contamination_exp_flag=FALSE,
                            save_plots=FALSE, reload=FALSE) {
  if(reload) {
    summary <- load_data(exp.num, contamination_exp_flag=plot.contamination_exp_flag)
  }
  
  init_settings(plot.contamination_exp_flag)
  ...
  if(plot.contamination_exp_flag){
    df.nominal_residual <- tibble(Key="frobenius_d", Mean=0)
  }else{
    df.nominal_residual <- tibble(Key="epsilon_res", Mean=0)
  }
  ...
  if(save_plots) {
    plot.file <- sprintf("figures/exp%d_%s_%s_picl%s.png",
                         exp.num, plot.data, plot.contamination, plot.pi_clean)
    ggsave(file=plot.file, height=2.5, width=9, units="in")
    return(NULL)
  } else{
    return(pp)
...
make_figure_822 <- function(exp.num,plot.data="cifar10",
  ...
  if(save_plots) {
    plot.file <- sprintf("figures/exp%d_%s_ncl%d_picl%s.png",
                         exp.num, plot.data, plot.n_clean, plot.pi_clean)
    ggsave(file=plot.file, height=2.5, width=9, units="in")
    return(NULL)
  } else{
    return(pp)
...
make_figure_822(exp.num=exp.num, plot.data=plot.data,
                ...
                plot.n_clean=plot.n_clean,
                plot.pi_clean=plot.pi_clean,
                plot.contamination_exp_flag=TRUE,
                save_plots=TRUE, reload=TRUE)
...
make_figure_912 <- function(exp.num, plot.alpha, plot.data="synthetic1", plot.guarantee="marginal",
  ...
  if(save_plots) {
    plot.file <- sprintf("figures/exp%d_%s_nt%d_ncl%d_eps%s_nu%s_optimistic%s.pdf",
                         exp.num, plot.data, plot.n_train, plot.n_clean, plot.epsilon, plot.nu, plot.optimistic)
    ggsave(file=plot.file, height=4.5, width=9, units="in")
    return(NULL)
  } else{
    return(pp)
...
init_settings <- function(contamination_exp_flag=FALSE) {
  ...
  if(contamination_exp_flag){
    method.values <<- c("NN gen", "NN light gen", "NN SLL gen")
    method.labels <<- c("NNc (g)", "NN (g)", "NNs (g)")
    color.scale <<- cbPalette[c(9,6,7)]
    shape.scale <<- c(9,3,4)
    linetype.scale <<- c(1,1,1)
  } else {
    method.values <<- c("NN", "NN light", "NN SLL")
    method.labels <<- c("NNc", "NN", "NNs")
    color.scale <<- cbPalette[c(3,5,4)]
    shape.scale <<- c(3,7,6)
    linetype.scale <<- c(1,1,1)
  }
...
plot.n_train <- 5000
plot.n_clean <- c(500)
make_figure_1101(exp.num=exp.num, plot.alpha=plot.alpha, plot.data=plot.data, plot.guarantee="marginal",
                plot.contamination=plot.contamination,
                plot.epsilon=plot.epsilon,
                plot.n_train=plot.n_train, plot.n_clean=plot.n_clean,
                save_plots=TRUE, plot.optimistic=TRUE, reload=TRUE)
...
#### Experiments 1103: Application to real contamination on BigEarthNet dataset ------------------------
init_settings <- function(plot.optimistic = FALSE) {
  df.dummy <<- tibble(key="Coverage", value=0.95)
  df.dummy2 <<- tibble(key="Coverage", value=0.5)
  cbPalette <<- c("grey50", "#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00", "#CC79A7", "#20B2AA", "#8A2BE2","#648767")
...
make_figure_1103 <- function(exp.num, plot.alpha, plot.data="bigearthnet", plot.guarantee="marginal",
                             plot.contamination="uniform",
                             plot.epsilon=0.1,
                             plot.nu=0,
                             plot.n_train=1000,
...
make_figure_1104 <- function(exp.num, plot.alpha, plot.data="bigearthnet", plot.guarantee="marginal",
                             plot.contamination="uniform",
                             plot.epsilon=0.1,
                             plot.nu=0,
                             plot.n_train=1000,
...
plot.n_clean <- c(500)
plot.guarantee="marginal"
make_figure_1104(exp.num=exp.num, plot.alpha=plot.alpha, plot.data=plot.data, plot.guarantee="marginal",
                 plot.contamination=plot.contamination,
                 plot.epsilon=plot.epsilon,
                 plot.n_train=plot.n_train, plot.n_clean=plot.n_clean,
                 save_plots=TRUE, plot.optimistic=TRUE, reload=TRUE)
...
init_settings <- function(plot.optimistic = FALSE) {
  ...
  method.labels <<- c("Standard",
                      ...
                      "",
                      "Standard (clean, simple)")
  color.scale <<- cbPalette[c(1,2,9,5,6,7,8,NA,10)]
  shape.scale <<- c(1,2,3,4,5,6,7,NA,NA)
  linetype.scale <<- c(1,1,1,1,1,1,1,0,4)
}
...
init_settings <- function(plot.optimistic = FALSE) {
  ...
  method.labels <<- c("Standard",
                      ...
                      "",
                      "Standard (clean, simple)")
  color.scale <<- cbPalette[c(1,2,6,7,8,NA,10)]
  shape.scale <<- c(1,2,5,6,7,NA,NA)
  linetype.scale <<- c(1,1,1,1,1,0,4)
}
