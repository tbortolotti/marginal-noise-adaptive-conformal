...
make_figure_A7(plot.epsilon=0.1, plot.nu=0.2, save_plots=TRUE, reload=FALSE)
...
make_figure_A8(plot.epsilon=0.1, plot.nu=0.2, save_plots=TRUE, reload=FALSE)

#' Plot 9: Finite sample correction as a function of the calibration set size,
#'         for different number of classes and for different combinations of epsilon and nu
#'

make_figure_A9 <- function(save_plots=FALSE, reload=FALSE, plot.epsilon=0.1, plot.nu.vals=c(0.2,0.8)) {
  if(reload) {
    summary <- load_data(1)
  }
...
make_figure_A9(plot.epsilon=0.1, plot.nu.vals=c(0.2,0.8), save_plots=TRUE, reload=FALSE)
...
#'         values of the calibration set size and for different combinations of epsilon and nu
#'         
make_figure_A10 <- function(save_plots=FALSE, reload=FALSE, plot.epsilon=0.1, plot.nu.vals=c(0.2,0.8)) {
  if(reload) {
    summary <- load_data(1)
  }
...
make_figure_A10(plot.epsilon=0.1, plot.nu.vals=c(0.2, 0.8), save_plots=TRUE, reload=FALSE)
...
make_figure_2 <- function(plot.alpha=0.1, plot.guarantee="marginal", save_plots=FALSE, reload=FALSE,
  ...
  if(save_plots) {
    plot.file <- sprintf("figures/new_synthetic1_ntrain%d_eps%f_nu%f_%s_%s.pdf", 10000, plot.epsilon, plot.nu, plot.guarantee, plot.contamination)
    ggsave(file=plot.file, height=3, width=7, units="in")
    return(NULL)
  } else{
    return(pp)
